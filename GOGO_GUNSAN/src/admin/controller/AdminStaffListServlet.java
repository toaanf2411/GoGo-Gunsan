...
/**
 * Servlet implementation class AdminStaffListServlet
 */
@WebServlet("/gogo-admin/adminStaffList")
public class AdminStaffListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//1.parameter handling
				...
				//2.business logic
				List<Staff> list = new StaffService().selectStaffList(cPage, numPerPage);
				
				//전체게시물수 totalContent
				int totalContents = new StaffService().selectTotalContents();
				...
				if(pageNo ==1 ) {
					...
					pageBar += "<a href='adminStaffList?cPage="+(pageNo-1)+"'>[이전]</a>";
				}
				//[pageNo]
				while(pageNo<=pageEnd && pageNo <= totalPage) {
					if(pageNo == cPage) {
						...
						pageBar += "<a href='adminStaffList?cPage="+pageNo+"'>"+pageNo+"</a>";				
					}
					pageNo++;
				}
				//[다음]
				...
				if(pageNo>totalPage) {
					...
					pageBar += "<a href='adminStaffList?cPage="+(pageNo)+"'>[다음]</a>";			
				}
				...
				request.getRequestDispatcher("/WEB-INF/views/admin/adminStaffList.jsp").forward(request, response);
	}
