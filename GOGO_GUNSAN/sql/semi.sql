...
select count(*) result from member where member_email = 'hong@naver.com';
...
--    drop sequence seq_fr_board_no;
    ...
    select  *  from ( select row_number() over(order by free_board_date desc) rnum, M.* from free_board M ) where rnum between 1 and 2;
...
select rownum, A.* from(select admin_board_no,
          admin_board_title,
          m.member_name,
          admin_board_content,
          admin_board_original_filename,
          admin_board_renamed_filename,
          admin_board_date,
          admin_board_readcount
from admin_board A join member m on a.admin_board_writer = m.member_no order by admin_board_date desc) A
